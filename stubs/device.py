...
def id(port: int) -> int:
    """
    Retrieve the device id of a device. Each device has an id based on its
    type.
...
def get_duty_cycle(port: int) -> int:
    """
    Retrieve the duty cycle for a device. Returned values is in range 0 to
    10000
...
def ready(port: int) -> bool:
    """
    When a device is attached to the hub it might take a short amount of time
    before it's ready to accept requests. Use `ready` to test for the readiness of the attached devices.
