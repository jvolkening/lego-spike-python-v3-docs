...
def acceleration(raw_unfiltered: bool) -> tuple[int, int, int]:
    ...
    integers. The values are mili G, so 1 / 1000 G
...
def get_yaw_face() -> int:
    ...
    If you put the hub on a flat surface with the face returned pointing
    up, when you rotate the hub only the yaw will update<br />
    ...
    Matrix.<br />
    `motion_sensor.RIGHT` The right side of the SPIKE Prime hub when facing
    the front hub face.<br />
    ...
    />
    `motion_sensor.LEFT` The left side of the SPIKE Prime hub when facing
    the front hub face.
...
def quaternion() -> tuple[float, float, float, float]:
    """Returns the hub orientation quaternion as a tuple[w: float, x:
...
def set_yaw_face(up: int) -> bool:
    ...
    face.<br />Available values are:`motion_sensor.TOP` The SPIKE Prime hub
    face with the USB charging port.<br />`motion_sensor.FRONT` The SPIKE
    Prime hub face with the Light Matrix.<br />`motion_sensor.RIGHT` The
    right side of the SPIKE Prime hub when facing the front hub face.<br
    />`motion_sensor.BOTTOM` The side of the SPIKE Prime hub where the
    battery is.<br />`motion_sensor.BACK` The SPIKE Prime hub face where
    the speaker is.<br />`motion_sensor.LEFT` The left side of the SPIKE
    Prime hub when facing the front hub face.
    :type up: int
    :rtype: bool
    """
...
def tap_count() -> int:
    """Returns the number of taps recognized since the program started or
    last time `motion_sensor.reset_tap_count()` was called.

    :rtype: int
    """

def tilt_angles() -> tuple[int, int, int]:
    """Returns a tuple containing yaw pitch and roll values as integers.
