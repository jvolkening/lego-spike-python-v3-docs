...
def run(*functions: Awaitable) -> None:
    """
    Start any number of parallel `async` functions. This is the function you
    should use to create programs with a similar structure to Word Blocks.
...
def until(function: Callable[[], bool], timeout: int = 0) -> Awaitable:
    """
    Returns an awaitable that will return when the condition in the function or
    lambda passed is `True` or when it times out
    ...
    :param function: A callable with no parameters that returns either
        `True` or `False`. Callable is anything that can be called, so a
        `def` or a `lambda`
