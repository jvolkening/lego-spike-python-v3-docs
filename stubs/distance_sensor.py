...
def distance(port: int) -> int:
    ...
    connected to `port`. If the Distance Sensor cannot read a valid distance it
    will return -1.
...
def get_pixel(port: int, x: int, y: int) -> int:
    """
    Retrieve the intensity of a specific light on the Distance Sensor connected
    to `port`.
...
def set_pixel(port: int, x: int, y: int, intensity: int) -> None:
    """
    Changes the intensity of a specific light on the Distance Sensor connected
    to `port`.
